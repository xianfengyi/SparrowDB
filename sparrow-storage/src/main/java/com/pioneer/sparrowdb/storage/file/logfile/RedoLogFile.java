...
public class RedoLogFile {
     ...
     * 事务开始时的偏移地址
     */
    private HashMap<Long, Long> txId2StartOffset = new HashMap<>();
    ...
    public RedoLogFile(File file) {
        ...
        try {
            ...
            throw new StorageException("load redo log file error ", e);
        }
    }
     ...
     */
    public synchronized void recordPageChange(TransactionID transactionID, Page beforePage, Page afterPage) throws IOException {
        raf.writeInt(LogType.PAGE_FLUSH);
        // 事务ID
        raf.writeLong(transactionID.getId());
        // 页数据
    ...
    public synchronized void recordTxCommit(TransactionID transactionID) throws IOException {
        raf.writeInt(LogType.TX_COMMIT);
        raf.writeLong(transactionID.getId());
        // 在本次记录的末尾处写入记录的开始位置
        raf.writeLong(offset);
        // 更新写指针
    ...
    public synchronized void rollback(long txIdRollback) throws IOException {
        ...
        while (txStartOffset <= offsetToRead) {
            ...
            int recordType = raf.readInt();
            if (recordType == LogType.PAGE_FLUSH) {
    ...
    private Page readPage() throws IOException {
        ...
        try {
            ...
            throw new StorageException("deserialize error", e);
        }
        return beforePage;
    }
