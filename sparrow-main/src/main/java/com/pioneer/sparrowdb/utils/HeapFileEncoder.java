package com.pioneer.sparrowdb.utils;

import com.pioneer.sparrowdb.storage.Type;
import com.pioneer.sparrowdb.storage.Utility;
...
public class HeapFileEncoder {
    ...
    public static void convert(File inFile, File outFile, int npagebytes, int numFields, Type[] typeAr,
        ...
        if (nheaderbytes * 8 < nrecords) nheaderbytes++;  //ceiling
        ...
        while (!done) {
            ...
            if (recordcount >= nrecords || done && recordcount > 0 || done && npages == 0) {
                ...
                for (i = 0; i < nheaderbits; i++) {
                    if (i < recordcount) headerbyte |= (1 << (i % 8));
